...
T RootishArrayStack<T>::get(int i) {
    int b = i2b(i);				// block number
    int j = i - b*(b+1)/2;      // index in the block
...
template<class T>  
void RootishArrayStack<T>::grow() { 	// Add another block
    blocks.add(blocks.size(), new T[blocks.size()+1]);
}

template<class T>
void RootishArrayStack<T>::shrink() { 	
	// if there are more than one empty block, remove all but one of unused blocks
    int r = blocks.size();
